...
// ../../packages/cli/src/scanner/index.ts
init_cjs_shims();

// ../../packages/cli/src/scanner/matchers.ts
init_cjs_shims();
var VALID_NAME = /^[A-Z_][A-Z0-9_]*$/;
var MATCHERS = [
  {
    ...
    resolve: (m) => {
      const name = m[1] ?? null;
      const confidence = name && VALID_NAME.test(name) ? "exact" : "exact";
...
var IGNORE_LINE_PATTERN = /\/\/\s*envmanifest-ignore-next-line\b/;
  ...
  for (const matcher of MATCHERS) {
    ...
    while ((m = matcher.pattern.exec(text)) !== null) {
      ...
      refs.push({
        ...
        ...resolved.note !== void 0 && { note: resolved.note }
      });
    }
  }
  refs.sort((a, b) => {
...
// ../../packages/cli/src/scanner/walk.ts
init_cjs_shims();
...
async function* walk(root, dir, exts, ignored) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      yield* walk(root, (0, import_node_path.join)(dir, name), exts, ignored);
      continue;
    }
    ...
    let info;
    try {
      info = await (0, import_promises.stat)(absolute);
    } catch {
      continue;
    }
    if (info.size > 5e6) continue;
    let content;
    try {
      content = await (0, import_promises.readFile)(absolute, "utf8");
    } catch {
      continue;
    }
    yield {
      absolute,
      relative: (0, import_node_path.relative)(root, absolute).split(import_node_path.sep).join("/"),
      content
    };
  }
}
...
// ../../packages/cli/src/manifest/load.ts
init_cjs_shims();
...
// ../../packages/cli/src/dotenv/parse.ts
init_cjs_shims();
var import_promises3 = require("fs/promises");
var import_node_path3 = require("path");
var LINE = /^\s*(?:export\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*=/;
async function loadDotenvFile(absPath) {
  let text;
  try {
    text = await (0, import_promises3.readFile)(absPath, "utf8");
  } catch (err) {
    if (err && typeof err === "object" && "code" in err && err.code === "ENOENT") {
      return null;
    }
...
async function discoverDotenvFiles(cwd, env) {
  const candidates = dotenvCandidatesFor(env);
  const searched = [];
  const found = [];
  for (const name of candidates) {
    const full = (0, import_node_path3.join)(cwd, name);
    searched.push(full);
...
function dotenvCandidatesFor(env) {
...
function reconcile(input2) {
  ...
  const dotenvNames = /* @__PURE__ */ new Set();
...
// ../../packages/cli/src/format/sarif.ts
init_cjs_shims();
var RULES = {
  "code.undeclared": {
    id: "code.undeclared",
...
function renderSarif(opts) {
  ...
  const usedRules = usedRuleIds.map((id) => RULES[id]).filter((r) => Boolean(r));
...
function setOutput(name, value) {
  ...
  void (0, import_promises4.appendFile)(path, `${name}=${value}
`, "utf8");
}
...
async function main() {
  ...
  const failOn = (input("fail-on", "error") || "error").toLowerCase();
  const cwd = (0, import_node_path4.isAbsolute)(manifestPath) ? (0, import_node_path4.dirname)(manifestPath) : (0, import_node_path4.dirname)((0, import_node_path4.resolve)(workspace, manifestPath));
  let manifest;
  try {
  ...
  if (format === "sarif") {
    const sarifPath = (0, import_node_path4.join)(workspace, "envmanifest.sarif");
    await (0, import_promises5.mkdir)((0, import_node_path4.dirname)(sarifPath), { recursive: true });
  ...
  const shouldFail = failOn === "warning" ? errors + warnings > 0 : failOn === "error" ? errors > 0 : false;
  if (shouldFail) {
    process.exitCode = 1;
  }
}
